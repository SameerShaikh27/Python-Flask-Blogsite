...
def contact():
    if request.method == "POST":
        ...
        entry = Contacts(name=name, phone_num=phone, msg=message, email_id=email, date=datetime.now())
...
def add(serial_no):
    if "user" in session and session["user"] == params["admin_user"]:
        if request.method == "POST":
            ...
            if serial_no == '0':
                post = Posts(title=title, tagline=tagline, content=content, slug=slug, img_file=img_file, date=date)
                db.session.add(post)
                db.session.commit()

        return render_template("add.html", params=params, serial_no=serial_no)
